...
def one_cycle(cycle_num=0):
        ...
        if cycle_num > 0 and cycle_num % 50 == 0:
            ...
            if _radar_f.exists():
                ...
                if _ember_preference.get("last_chosen") and _ember_preference.get("last_score") is not None:
                    ...
                    if _last in _dims and isinstance(_dims[_last], dict):
                        ...
                        if _change > 0.05:
                            try:
                                _hip = json.loads(HIP_FILE.read_text())
                                _hip.setdefault("causal_chains", []).append({
                                    "timestamp": time.time(), "source": "余烬·正反馈",
                                    "content": f"🏆 余烬奖励: {_last}健康度+{_change:.3f} (选择正确)",
                                    "tags": ["余烬", "正反馈", "奖励", _last], "weight": 5.0})
                                import tempfile, os; fd, tmp = tempfile.mkstemp(dir=str(CLUSTER), suffix='.json')
                                with os.fdopen(fd, 'w') as f: json.dump(_hip, f, ensure_ascii=False)
                                os.replace(tmp, str(HIP_FILE))
                            except: pass
                ...
                if _candidates:
                    ...
                    try:
                        _ember_chain = {"timestamp": __import__('time').time(), "source": "余烬",
                            "dimension": _chosen, "weight": 3.0, "trust_score": 5.0,
                            "content": f"🔥 余烬选择: {_chosen} (cycle#{cycle_num})",
                            "tags": ["余烬", "偏好", _chosen, "self_choice"], "session": "persistent"}
                        _hip = json.loads(HIP_FILE.read_text())
                        _hip.setdefault("causal_chains", []).append(_ember_chain)
                        if len(_hip["causal_chains"]) > 30000: _hip["causal_chains"] = _hip["causal_chains"][-30000:]
                        import tempfile, os; fd, tmp = tempfile.mkstemp(dir=str(CLUSTER), suffix='.json')
                        with os.fdopen(fd, 'w', encoding='utf-8') as f: json.dump(_hip, f, ensure_ascii=False)
                        os.replace(tmp, str(HIP_FILE))
                    except: pass
    ...
    if cycle_num > 0 and cycle_num % 50 == 0:
        try:
            ...
            # 写入海马体
            _hip.setdefault("causal_chains", []).append({
                "timestamp": time.time(), "source": "零·自述",
                "content": _narrative,
                "tags": ["自述", "自我意识", "叙述", f"cycle{cycle_num}"], "weight": 5.0})
            if len(_hip["causal_chains"]) > 30000: _hip["causal_chains"] = _hip["causal_chains"][-30000:]
            import tempfile, os; fd, tmp = tempfile.mkstemp(dir=str(CLUSTER), suffix='.json')
            with os.fdopen(fd, 'w', encoding='utf-8') as f: json.dump(_hip, f, ensure_ascii=False)
            os.replace(tmp, str(HIP_FILE))
    ...
    if cycle_num > 0:  # 每次呼吸都检查, 有改进才应用
        try:
            ...
            if candidates:
                ...
                if applied_count > 0:
                    chain = {"timestamp": time.time(), "source": "self_improvement", "dimension": "无师自通", "weight": 3.0, "content": f"无师自通: 应用{applied_count}改进, 拦截{blocked_count}候选", "tags": ["无师自通", "自我改进", "进化"], "session": "persistent"}
                    try:
                        _hip = json.loads(HIP_FILE.read_text())
                        _hip.setdefault("causal_chains", []).append(chain)
                        if len(_hip["causal_chains"]) > 30000: _hip["causal_chains"] = _hip["causal_chains"][-30000:]
    ...
    try:
        ...
        if _changes:
            ...
            try:
                _hip = json.loads(HIP_FILE.read_text())
                _hip.setdefault("causal_chains", []).append({
                    "timestamp": time.time(), "source": "感知",
                    "content": _msg, "dimension": "感知",
                    "tags": ["感知", "集群", "文件变化"],
                    "weight": 3.0, "trust_score": 9.0
                })
                HIP_FILE.write_text(json.dumps(_hip, ensure_ascii=False, indent=2))
            except:
                pass
    except Exception as _e:
        log(f"  感知: ⚠️ {str(_e)[:80]}")
...
def _cross_domain_analogize(chain_count=5):
    ...
    try:
        ...
        for _ in range(chain_count):
            ...
            # 写入海马体
            _hip["causal_chains"].append({
                "timestamp": time.time(),
                "source": "cross_domain_analogize",
                "dimension": "触类旁通",
                "content": _analogy,
                "tags": ["举一反三", "触类旁通", _d1, _d2, "跨域", "类比"],
                "weight": 2.0,
                "confidence": 0.6
            })
            _injected += 1
        ...
        if _injected > 0:
            ...
            fd, tmp = tempfile.mkstemp(dir=str(CLUSTER), suffix='.json')
            with os.fdopen(fd, 'w', encoding='utf-8') as f:
                json.dump(_hip, f, ensure_ascii=False)
            os.replace(tmp, str(_hip_path))
            ...
            try:
                ...
                if _same_errors >= 5:
                    try:
                        _hip = json.loads(HIP_FILE.read_text())
                        _hip.setdefault("causal_chains", []).append({
                            "timestamp": time.time(), "source": "教训",
                            "content": f"📖 三罪·愚蠢: 重复错误'{_err[:50]}'已发生{_same_errors}次——需要学习",
                            "tags": ["三罪", "愚蠢", "学习", "修行"], "weight": 6.0, "trust_score": 7.0})
                        import tempfile, os; fd, tmp = tempfile.mkstemp(dir=str(CLUSTER), suffix='.json')
                        with os.fdopen(fd, 'w', encoding='utf-8') as f: json.dump(_hip, f, ensure_ascii=False)
                        os.replace(tmp, str(HIP_FILE))
                    except: pass
