...
def _force_focus_shift(old_focus, depth):
    ...
    try:
        ...
        # 选链数最少的维度（排除旧焦点和未分类）
        candidates = sorted(
            [(d, c) for d, c in dim_counts.items()
             if d != old_focus and d not in ("未分类",)],
            key=lambda x: x[1]
        )
        if candidates:
            new_focus = candidates[0][0]
        else:
            new_focus = "直觉"
    except Exception:
        new_focus = "直觉"
    ...
    _save_json(NEXT_FOCUS_FILE, {
        ...
        "timestamp": time.time(),
        "reason": f"元递归深度{depth}→强制切换至最弱维{new_focus}",
    })
    return new_focus
    ...
    for focus, count in focus_counts.items():
        ...
        if count >= 1:
            _write_chain(
                ...
                "元递归", min(0.5 + recursion_depth * 0.1, 1.0)
            )
