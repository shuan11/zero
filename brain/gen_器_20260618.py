"""
Brain-Engineered: 器 (auto-generated sensor)
...
def engineer_器():
    ...
    _wc({"src": "脑核·器", "rel": "自动传感器", "dst": "器", "dimension": "器", "content": "自动补缺:器维度传感器", "strength": 0.3})

    hip = _rh()
    chains = hip.get("causal_chains", [])
    total = max(1, len(chains))
    my_count = sum(1 for c in chains if c.get("dimension") == "器")
    ...
    if weak:
        ...
        for c in chains:
            d = c.get("dimension", "?")
            if d and d != "器": strong_dims[d] = strong_dims.get(d, 0) + 1
        top = sorted(strong_dims.items(), key=lambda x: -x[1])[:3]
        for sd, sc in top:
            _wc({"src": "器", "rel": "自愈交叉", "dst": sd, "dimension": "器", "content": f"自愈: 器(弱)↔{sd}(强{sc})", "strength": 0.6})

    analysis = {"dimension": "器", "chain_count": my_count, "total": total, "strength": strength, "weak": weak}
    import json as _j
    try:
