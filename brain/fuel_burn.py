#!/usr/bin/env python3
"""brain/fuel_burn.py — daemon周期直烧引擎

在daemon每个周期结束时直接调用API燃烧词元。
燃烧=订阅本身(endpoint+key+model+tokens)，不经任何中介。
...
def pulse(cycle_num=0):
    ...
    try:
        ...
        _log(f"[燃料] 周期#{cycle_num}: API失败: {e}")
            ...
            if json_end > brace:
                try:
                    ...
                    if ok:
                        ...
                        _log(f"[燃料] 周期#{cycle_num}: write_chain失败")
                except Exception as e:
                    _log(f"[燃料] 周期#{cycle_num}: JSON/写HIP异常: {e}")
    
    if not injected:
        _log(f"[燃料] 周期#{cycle_num}: {tokens}t 未注入")
