"""
Brain-Engineered: 活着 (generation 1782250477771)
管道自动检测弱维<活着>并生成v3工程
"""
...
def engineer_活着():
    ...
    try:
        ...
        chains = []
    dim_count = sum(1 for c in chains if c.get("dimension") == "活着")
    total = len(chains)
    ...
    try:
        ...
        _other_counts = [c for d,c in _all_dims.items() if d != "活着" and d not in ("系统","未分类")]
        if _other_counts:
    ...
    try:
        ...
        # 4a. 弱维时注册调优动作
        if _is_weak:
            _ra("update_genome", {"changes": {}, "dimension": "活着",
                "reason": f"{dim_name}偏弱({dim_count}/max={_max_other})"},
                priority=5, source="gene:活着")
        ...
        if not _is_weak and dim_count > 0:
            _ra("write_chain", {"src": f"巩固·活着",
                "rel": f"基因表达·#1782250477771", "dst": "活着",
    ...
    if _is_weak:
        return f"[弱] 活着={dim_count}/{total} (max={_max_other})"
    return f"[稳] 活着={dim_count}/{total}"
