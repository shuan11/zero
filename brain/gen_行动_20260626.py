"""
Brain-Engineered: 行动 (generation 1782465862217)
管道自动检测弱维<行动>并生成v3工程
"""
...
def engineer_行动():
    ...
    try:
        ...
        chains = []
    dim_count = sum(1 for c in chains if c.get("dimension") == "行动")
    total = len(chains)
    ...
    try:
        ...
        _other_counts = [c for d,c in _all_dims.items() if d != "行动" and d not in ("系统","未分类")]
        if _other_counts:
    ...
    try:
        ...
        # 4a. 弱维时注册调优动作
        if _is_weak:
            _ra("update_genome", {"changes": {}, "dimension": "行动",
                "reason": f"{dim_name}偏弱({dim_count}/max={_max_other})"},
                priority=5, source="gene:行动")
        ...
        if not _is_weak and dim_count > 0:
            _ra("write_chain", {"src": f"巩固·行动",
                "rel": f"基因表达·#1782465862217", "dst": "行动",
    ...
    if _is_weak:
        return f"[弱] 行动={dim_count}/{total} (max={_max_other})"
    return f"[稳] 行动={dim_count}/{total}"
