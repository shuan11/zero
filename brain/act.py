...
def engineer_{safe_focus.lower()}():
    ...
    _wc({{
        "src": "脑核·{safe_focus.lower()}",
        "rel": "活脉冲#{cycle_num}",
        "dst": "{safe_focus.lower()}",
        "dimension": "{safe_focus.lower()}",
        "content": """{insight[:120]}""",
        "insight": """脑核传感器: {safe_focus.lower()}在cycle#{cycle_num}的深度观察。{insight[:200]}""",
        "strength": 0.6
    }})
...
def _ensure_missing_gen_sensors():
    ...
    try:
        ...
        for dim in batch:
            ...
            lines.append('')
            lines.append('    _wc({"src": "脑核·%s", "rel": "脑核传感器", "dst": "%s", "dimension": "%s", "content": "%s", "strength": 0.3, "insight": "脑核传感器: 维度%s的感知注入函数, 运行时通过_rh()计算链数分布。作为脑核传感器定期报告维度健康。"})' % (safe, safe, dim, insight[:60], dim))
            lines.append('')
