...
HIPPOCAMPUS_DIM_MAP = {
    ...
    "通知": ["通知", "通知链", "SYSTEM通知", "notification", "background", "notify"],
    "自我通知": ["自我通知", "自通知", "self-notify", "self通知", "自主通知", "自驱动通知"],
    "最弱维": ["最弱维", "弱维", "weakest", "最弱维度", "weak-dim"],
    "维演化": ["维演化", "维度演化", "dimension-evolution", "维度变迁", "维收敛"],
...
def write_chain(chain):
    ...
    try:
        ...
        if dim not in dims:
            dims[dim] = {"chain_count": 1, "strength": 0.5}
        else:
            dims[dim]["chain_count"] = sum(1 for c in cats if c.get("dimension") == dim)
        data["metadata"] = {
